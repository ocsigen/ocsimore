...
module Persist = struct
  ...
  let lwtcreate db name default =
		Messages.debug2 "[Sql] lwtcreate";
		begin_work db >>= 
		fun () -> LWT_PGSQL(db) "SELECT value FROM globalstore WHERE key = $name" >>=
    ...
    function
      ...
      | None -> default () >>=
          (fun def ->
	        ...
	        LWT_PGSQL(db) "INSERT INTO globalstore VALUES ($name, $v)" >>=
          fun () -> commit db >>=
  ...
  let set db (pvname, pvref) value =
    ...
    pvref := value;
    LWT_PGSQL(db) "UPDATE globalstore SET value = $v WHERE key = $pvname"
...
let connect () =
	Messages.debug2 "[Sql] connect: start";
...
let new_forum db ~title ~descr ~moderated =
  (* inserts a new forum *)
  begin_work db >>=
  fun () -> LWT_PGSQL(db) "INSERT INTO forums (title, descr, moderated) \
...
let new_thread_and_message db ~frm_id ~author ~subject ~txt = 
  (* inserts a message starting a new thread; both thread and message
	...
	fun hidden -> LWT_PGSQL(db) "INSERT INTO threads (frm_id, subject, hidden, author) \
	...
	fun db_max -> LWT_PGSQL(db) "INSERT INTO messages (author, thr_id, txt_id, hidden, tree_min, tree_max) \
...
let new_thread_and_article db ~frm_id ~author ~subject ~txt =
	...
	fun () -> serial4 db "textdata_id_seq" >>=
	fun txt_id -> LWT_PGSQL(db) "INSERT INTO threads (frm_id, subject, hidden, author, article_id) \
...
let new_message db ~thr_id ?parent_id ~author ~txt ~sticky () = 
  (* inserts a message in an existing thread; message will be hidden
	...
	fun db_max -> 
		...
		| None -> LWT_PGSQL(db) "INSERT INTO messages (author, thr_id, txt_id, hidden, \
		...
		| Some pid -> LWT_PGSQL(db) "SELECT tree_min, tree_max \
				...
				fun () -> LWT_PGSQL(db) "INSERT INTO messages (author, thr_id, txt_id, hidden, \
					sticky, tree_min, tree_max) \
...
let thread_get_data db (* ~frm_id *) ~thr_id ~role =
  (* returns id, subject, author, datetime, hidden status, number of
     ...
     thread. *)
	begin_work db >>=
	fun () -> LWT_PGSQL(db) "SELECT t.id, subject, t.author, t.datetime, t.hidden, COALESCE (txt, '') \
...
let message_get_data db ~frm_id ~msg_id =
  (* returns id, text, author, datetime, hidden status of a message *)
	LWT_PGSQL(db) "SELECT messages.id, textdata.txt, messages.author, \
...
| None -> 
	...
	fun () -> (match role with 
	| Moderator ->
		LWT_PGSQL(db) "SELECT messages.id,txt,author,datetime,hidden,sticky \
	...
	fun () -> let final_msg_l = match bottom with
		...
		| Some btm -> cut (fun (id,_,_,_,_,_) -> id) btm msg_l in
			Messages.debug2 "[Sql] thread_get_messages_with_text: end 1";
			return final_msg_l
| Some x -> let db_limit = db_size_of_int x in
	begin_work db >>=
	fun () -> (match role with 
...
let db_limit = db_size_of_int limit in
	LWT_PGSQL(db) "SELECT messages.id,txt,author \
	FROM messages, textdata \
