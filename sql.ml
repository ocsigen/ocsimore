...
let db = connect ~database:"ocsigen" ()
...
module Persist = struct
  ...
  let create name default =
    begin_work(db);
    let pv = 
      match PGSQL(db) "SELECT value FROM globalstore WHERE key = $name" with
	| [Some n] -> (name, ref (Marshal.from_string n 0)) 
	| _ -> 
	    let def = default () in
...
let wiki_get_data ~wik_id = 
  ...
  let (title, description) = 
    (match PGSQL(db) "SELECT title, descr FROM wikis WHERE id = $wik_id"
     with [x] -> x | _ -> assert false) in
  let n_pages = 
    (match PGSQL(db) "SELECT COUNT(*) FROM wikipages WHERE wik_id = $wik_id"
     with [Some x] -> x | _ -> assert false) in
    commit db;
