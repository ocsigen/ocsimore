...
let new_message db ~thr_id ?parent_id ~author_id ~txt ~sticky () = 
  (* inserts a message in an existing thread; message will be hidden
	...
	begin_work db >>=
	fun _ -> PGSQL(db) "SELECT moderated FROM forums,threads \
	...
	fun db_max -> 
		...
		| None -> PGSQL(db) "INSERT INTO messages (author_id, thr_id, txt_id, hidden, \
		...
		| Some pid -> PGSQL(db) "SELECT tree_min, tree_max \
				...
				fun () -> PGSQL(db) "INSERT INTO messages (author_id, thr_id, txt_id, hidden, \
					sticky, tree_min, tree_max) \
...
let find_forum db ?id ?title () =
 begin_work db >>= fun _ -> 
 (match (title, id) with
    | (Some t, Some i) ->  PGSQL(db) "SELECT forums.id, title, r.login, w.login, m.login \
    ...
    | (Some t, None) -> PGSQL(db) "SELECT forums.id, title, r.login, w.login, m.login \
		FROM forums, users AS r, users AS w, users AS m \
		WHERE r.id = reader AND w.id = writer AND m.id = moderator \
		AND title = $t"
    | (None, Some i) -> PGSQL(db) "SELECT forums.id, title, r.login, w.login, m.login \
		FROM forums, users AS r, users AS w, users AS m \
		WHERE r.id = reader AND w.id = writer AND m.id = moderator \
		AND forums.id = $i"
...
let get_forums_list db =
	...
	begin_work db >>=
	fun _ -> PGSQL(db) "SELECT id, title, descr, moderated, arborescent FROM forums" >>=
	fun r -> commit db >>=
...
let forum_get_data db ~frm_id ~role =
  (* returns id, title, description, mod status, number of shown/hidden
	...
	begin_work db >>=
  fun _ -> PGSQL(db) "SELECT id, title, descr, moderated FROM forums \
		WHERE id = $frm_id" >>=
	fun y -> (match y with [x] -> return x | _ -> fail Not_found) >>=
...
let thread_get_data db (* ~frm_id *) ~thr_id ~role =
  (* returns id, subject, author, datetime, hidden status, number of
     ...
     thread. *)
	begin_work db >>=
	fun _ -> PGSQL(db) "SELECT t.id, subject, fullname, t.datetime, t.hidden, COALESCE (txt, '') \
...
let thread_get_neighbours db ~frm_id ~thr_id ~role =
  (* returns None|Some id of prev & next thread in the same forum. *)
	...
	begin_work db >>=
	fun _ -> PGSQL(db) "SELECT datetime FROM threads \
		WHERE id = $thr_id AND frm_id = $frm_id" >>=
	fun y -> (match y with [x] -> return x | _ -> fail Not_found) >>=
...
let message_get_neighbours db ~frm_id ~msg_id ~role =
  (* returns None|Some id of prev & next message in the same
	...
	begin_work db >>=
	fun _ -> PGSQL(db) "SELECT messages.thr_id, messages.datetime \
...
let forum_get_threads_list db ~frm_id ?offset ?limit ~role () =
  (* returns the threads list of a forum, ordered cronologycally
	...
	| None -> 
		(match role with
		| Moderator -> PGSQL(db) "SELECT threads.id, subject, fullname, datetime, hidden \
				FROM threads, users \
				WHERE frm_id = $frm_id AND author_id = users.id \
				ORDER BY datetime DESC \
				OFFSET $db_offset"
		| Author aid ->
			PGSQL(db) "SELECT threads.id, subject, fullname, datetime, hidden \
				FROM threads, users \
				WHERE frm_id = $frm_id \
				AND (author_id = $aid OR NOT hidden) \
				...
				ORDER BY datetime DESC \
				OFFSET $db_offset"
		| Unknown -> PGSQL(db) "SELECT threads.id, subject, fullname, datetime, hidden \
				FROM threads, users \
