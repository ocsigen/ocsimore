...
let add_debug_expr name e =
  ...
  let msg = name ^ ": " ^ (Loc.to_string _loc) ^ "@." in
    (<:expr< prerr_endline $str:String.escaped msg$; $e$ >>)
...
let rec map_str_item = function
  | Ast.StVal(_loc, rec_mode,
              (Ast.BiEq (_, (Ast.PaId (_, (Ast.IdLid (_, f)))),
                         ((Ast.ExFun _ as e))))) ->
      (<:str_item< let $rec:rec_mode$ $lid:f$ = $map_expr f e$ >>)
  | x -> x
...
let _ =
  AstFilters.register_str_item_filter (fun x -> let _loc = Ast.loc_of_str_item x in <:str_item< $ x $ ;; Printf.eprintf $str:"********" ^ (Loc.file_name ( Ast.loc_of_str_item x )) ^"\n%!"$ >>)
