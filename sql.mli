...
val connect : ?host:string -> ?port:int -> ?user:string -> ?password:string -> ?database:string -> ?unix_domain_socket_dir:string -> unit -> 'a t Lwt.t
...
val begin_work : 'a t -> unit Lwt.t
(** Start a transaction. *)

val commit : 'a t -> unit Lwt.t
(** Perform a COMMIT operation on the database. *)

val rollback : 'a t -> unit Lwt.t
(** Perform a ROLLBACK operation on the database. *)

(** {6 Serial column} *)

val serial : 'a t -> string -> int64 Lwt.t
...
val serial4 : 'a t -> string -> int32 Lwt.t
(** As {!PGOCaml.serial} but assumes that the column is a SERIAL or
  * SERIAL4 type.
  *)

val serial8 : 'a t -> string -> int64 Lwt.t
...
val prepare : 'a t -> query:string -> ?name:string -> ?types:oid list -> unit -> unit Lwt.t
...
val execute : 'a t -> ?name:string -> ?portal:string -> params:param list -> unit -> row list Lwt.t
...
val close_statement : 'a t -> ?name:string -> unit -> unit Lwt.t
(** [close_statement conn ?name ()] closes a prepared statement and frees
  * up any resources.
  *)

val close_portal : 'a t -> ?portal:string -> unit -> unit Lwt.t
...
val describe_statement : 'a t -> ?name:string -> unit -> (params_description * row_description option) Lwt.t
(** [describe_statement conn ?name ()] describes the named or unnamed
  * statement's parameter types and result types.
  *)

val describe_portal : 'a t -> ?portal:string -> unit -> row_description option Lwt.t
