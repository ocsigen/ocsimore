...
let update_wiki ~(rights : Wiki_types.wiki_rights) ~sp ?container ?staticdir ?path ?descr ?boxrights ?model wiki =
  if staticdir <> None || path <> None || boxrights <> None || model <> None then
    rights#can_create_wiki sp () >>= function
      | true -> Wiki_sql.update_wiki
          ?container ?staticdir ?path ?descr ?boxrights ?model wiki
      | false -> Lwt.fail Ocsimore_common.Permission_denied
  else
